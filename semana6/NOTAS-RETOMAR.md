# Semana 6 — Notas para retomar el proyecto

Resumen del estado del trabajo (Gamer's Heaven + Spring + H2 / Oracle). Úsalo cuando vuelvas a continuar.

## Proyecto

- **Ruta del módulo Maven:** carpeta `semana6` donde está el `pom.xml`.
- **Spring Boot:** 4.0.5 · **Java:** 21.
- **Caso:** tienda "Gamer's Heaven" — API REST para listar / buscar videojuegos.
- **Paquetes principales:** `com.duoc.semana6.videojuegos` → `entity`, `dto`, `repository` (JPA), `service`, `controller` (alineado al estilo del ejemplo `ms-usuarios`).
...
- La **respuesta de la API** sigue basada en el **catálogo en memoria** (`ArrayList` en `VideojuegoService`), como pide la actividad formativa.
- Con perfil **`h2`**, al arranque también se persisten esos mismos ítems en H2 con **`videojuegoRepository.saveAll(...)`**, para que la tabla `VIDEOJUEGOS` tenga datos en la consola H2.
...
cd "C:\Users\lisbe\OneDrive\Escritorio\semana6\semana6"
